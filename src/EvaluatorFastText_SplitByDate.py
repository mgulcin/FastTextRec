...
def prune_users(user_2_list_of_visits_single, trainDataFilePath):
    ...
    pruned_users_2_list_of_visits_single = {
        user: user_2_list_of_visits_single[user] for user in user_2_list_of_visits_single.keys() if user in all_jan_users}
...
def read_true_visits_seq(test_pruned_output_file_path):
    ...
    with open(test_pruned_output_file_path, "r") as f:
        ...
        for row in reader:
            ...
            list_of_visits_seq = updated_row[2:] # 1st is used in training
            # remove all bust last item (used in training)
            #list_of_visits_seq = updated_row[-1]
...
def read_rec_seq(rec_seq_output_folder, rec_output_file):
    ...
    with open(rec_seq_output_file_path, "r") as f:
...
def read_rec_single(rec_output_folder, rec_output_file):
    ...
    with open(rec_output_file_path, "r") as f:
        reader = csv.reader(f, delimiter=',')#, quotechar = "'"
        for row in reader:
            # remove ' and space
            updated_row = [value.replace("'", "") for value in row]
...
def calculateDcg(rec_size, y_true, y_pred):
    ...
    if isRelevant(y_pred[0], y_true):
        rel1 = 1.0
    total = 0.0
    for i in range(1, min(rec_size, len(y_pred))):
        rel = 0.0
...
def find_overall_hitrate(user_list, user_2_tp_and_fp_and_prec_and_ndcg):   
    ...
    for user, input_tuple in user_2_tp_and_fp_and_prec_and_ndcg.items():
...
def run(selected_users, rec_size, baseRecommendationOutputFileName, rec_output_file_acronomies_list, rec_output_type,
        recommendationOutputFolderBaseSeq,
    ...
    for rec_output_file_acronomy in rec_output_file_acronomies_list:
        ...
        user_2_rec_seq, user_2_rec_seq_to_single, user_2_rec_seq_to_first = read_rec_seq(
            recommendationOutputFolderBaseSeq, rec_output_file_seq)
...
if __name__ == '__main__':
    ...
    base_dir = "XX"
    dataFolder = "XX/dataset/checkins2011_janjuly/"
    ...
    evaluationOutputFilePathBase = base_dir +"output/output_janjuly/fasttext_contentfilter/"
    ...
    all_users = user_2_list_of_visits_single.keys()
    only_train_users = prune_users(user_2_list_of_visits_single, trainDataFilePath)
    only_test_users = list(set(all_users) - set(only_train_users))

    # # select users: who has check-in in both training and testing
    # selected_users = only_train_users
    # evaluationOutputFilePath = evaluationOutputFilePathBase + "evaluationResult_onlytrain.txt"

    # # all
    # selected_users = all_users
    # evaluationOutputFilePath = evaluationOutputFilePathBase + "evaluationResult_all.txt"

    # only test
    selected_users = only_test_users
    evaluationOutputFilePath = evaluationOutputFilePathBase + "evaluationResult_onlytest.txt"
